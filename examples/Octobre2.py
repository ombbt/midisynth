...
scenerun2 =[
	pads1,
	ChannelSplit({
		1: KeySplit('C3', Velocity(fixed=40) >> Transpose(22) >> [out2, talkboxdetect], Velocity(fixed=40) >> Filter(~PITCHBEND) >> Filter(~CTRL) >> Transpose(10) >> out1 ),
	#	2: Transpose(10) >> out2,
	#	3: Transpose(10) >> out3,
...
scenerun3 =[
	pads1,
	ChannelSplit({
		1: KeySplit('C3', Velocity(fixed=40) >> Transpose(19) >>[out2, talkboxdetect], Velocity(fixed=40) >> Filter(~PITCHBEND) >> Filter(~CTRL) >> Transpose(7) >> out1),
		4: MakeMonophonic() >> Transpose(-5) >> out4,
		10: Pass() >> out10,
})
]
